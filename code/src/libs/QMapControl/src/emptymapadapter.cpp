...
namespace qmapcontrol
{
    ...
    {
        qreal x = (coordinate.x()+180.0) * (numberOfTiles*mytilesize)/360.; // coord to pixel!
        qreal y = (1.0 - (log(tan(PI*0.25+deg_rad(coordinate.y())*0.5)) /PI)) *0.5  * (numberOfTiles*mytilesize);
    ...
    {
        qreal longitude = (point.x()*(360.0/(numberOfTiles*mytilesize)))-180.0;
        qreal latitude = rad_deg(atan(sinh((1.0-point.y()*(2/(numberOfTiles*mytilesize)))*PI)));
