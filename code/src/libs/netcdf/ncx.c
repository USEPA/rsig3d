...
static void swapn4b( void* output, const void* input, size_t count ) {
  const unsigned int* const int_input = input;
  unsigned int* const int_output = output;
  long long index = 0; /* OpenMP requires a signed loop index. */
#if 0
  assert( output ); assert( input );
  assert( sizeof (int) == 4 ); assert( sizeof (long long) == 8 );
  assert( count <= 9223372036854775807LL );
#endif
...
#pragma omp parallel for
  for ( index = 0; index < count; ++index ) {
    const unsigned int value = int_input[ index ];
    int_output[ index ] =
      ( value & 0xff000000 ) >> 24 |
      ( value & 0x00ff0000 ) >>  8 |
      ( value & 0x0000ff00 ) <<  8 |
