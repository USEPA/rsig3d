...
class VectorMap
{
 ...
 private:
    ...
    void lambertconformalconicCreateFromPntMap( const PntMap* pntmap, const ViewportParams* viewport );
	void stereographicCreateFromPntMap( const PntMap* pntmap, const ViewportParams* viewport );
	
    void lambertconformalconicCreatePolyLine(qreal proj_n, qreal proj_F, qreal proj_rho0,
                                             const LambertConformalConicProjection *lambert_proj,
    ...
    void stereographicCreatePolyLine(qreal proj_n, qreal proj_F, qreal proj_rho0,
									 const StereographicProjection *lambert_proj,
									 GeoDataCoordinates::Vector::ConstIterator const & itStartPoint,
									 GeoDataCoordinates::Vector::ConstIterator const & itEndPoint,
									 const int detail, const ViewportParams *viewport, int offset);
