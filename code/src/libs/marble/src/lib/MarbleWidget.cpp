...
int marblelib_initialized=0;

int init_marblelib(void)
{
	if (marblelib_initialized==0){
	#ifdef USE_INTERNAL_MARBLE
		Q_INIT_RESOURCE(marble);
		Q_INIT_RESOURCE(libmarble);
		extern int kml_init(void);
		extern int dgml_init(void);
		kml_init();
		dgml_init();
	#endif
		marblelib_initialized = 1;
		qDebug()<<"Marble library is initialized.";
	}
	return marblelib_initialized;
}
...
class MarbleWidgetPrivate
{
 public:
    MarbleWidgetPrivate( MarbleWidget *parent )
        : m_widget( parent ),
		  m_libInitialized(init_marblelib()),
          m_model(),
          m_map( &m_model ),
          m_animationsEnabled( false ),
          m_logzoom( 0 ),
          m_zoomStep( MarbleGlobal::getInstance()->profiles() & MarbleGlobal::SmallScreen ? 20 : 10 ),
          m_inputhandler( 0 ),
          m_physics( parent ),
          m_routingLayer( 0 ),
