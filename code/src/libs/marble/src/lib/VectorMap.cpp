                                  ...
                                  const ViewportParams* viewport )
{
    switch( viewport->projection() ) {
		...
		case LambertConformalConic:
            lambertconformalconicCreateFromPntMap( pntmap, viewport );
			break;
		case Stereographic:
            stereographicCreateFromPntMap( pntmap, viewport );
			break;
    }
}


void VectorMap::stereographicCreateFromPntMap( const PntMap* pntmap, 
											   const ViewportParams* viewport )
{
    const StereographicProjection* projection =
	dynamic_cast<const StereographicProjection*>(viewport->currentProjection());
    if (projection == NULL){
		qDebug()<<"Error: projection is NULL, StereographicProjection is needed!";
		return;
	}
	qreal proj_n, proj_F, proj_rho0;
    ...
    for (; itPolyLine != itEndPolyLine; ++itPolyLine )
    {
		...
		stereographicCreatePolyLine(proj_n, proj_F, proj_rho0, projection,
									(*itPolyLine)->constBegin(), (*itPolyLine)->constEnd(), detail, viewport, offset );					
    }
}


void VectorMap::stereographicCreatePolyLine(qreal proj_n, qreal proj_F, qreal proj_rho0,
											const StereographicProjection *projection,
...
{	
    ...
    static ScreenPolygon otherPolygon;
	if (otherPolygon.capacity()<256) otherPolygon.reserve(256);
	otherPolygon.resize(0);
    ...
    for (; itPoint != itEndPoint; ++itPoint ) {
        ...
        if ( lastSign != currentSign && fabs(lastLon) + fabs(lon) > M_PI ) {
			...
			qreal lastXAtDateLine = lastSign * M_PI;
			qreal xAtDateLine = -lastXAtDateLine;
			qreal lastYAtDateLine = lastLat;
			qreal yAtSouthPole = -viewport->currentProjection()->maxLat();
	...
	static ScreenPolygon untransformedPolygon;
	if (untransformedPolygon.capacity()<256) untransformedPolygon.reserve(256);
	
	ScreenPolygon & src = m_polygon;
    if ( src.size() >= 2 ) {
		ScreenPolygon& m = untransformedPolygon;		
		m.resize(0);
		m.setClosed( src.closed() );
		for (int i=0; i<src.size(); i++){
			const QPointF v = src[i];
			qreal x0, y0;
			projection->screenCoordinates(v.x(), v.y(), viewport, x0, y0);
			m << QPointF (x0, y0);
		}
        m_polygons.append(m);
    }
...
void VectorMap::lambertconformalconicCreateFromPntMap( const PntMap* pntmap, 
										  const ViewportParams* viewport )
{
	const LambertConformalConicProjection* lambertProjection = 
	    dynamic_cast<const LambertConformalConicProjection*>(viewport->currentProjection());
    if (lambertProjection == NULL){
    ...
    for (; itPolyLine != itEndPolyLine; ++itPolyLine )
    {
		...
		m_polygon.reserve( (*itPolyLine)->size() );
		m_polygon.setClosed( (*itPolyLine)->getClosed() );
		lambertconformalconicCreatePolyLine(proj_n, proj_F, proj_rho0, lambertProjection,
											(*itPolyLine)->constBegin(), (*itPolyLine)->constEnd(), detail, viewport, offset );					
    }
}

void VectorMap::lambertconformalconicCreatePolyLine(qreal proj_n, qreal proj_F, qreal proj_rho0,
													const LambertConformalConicProjection *lambertProjection,
													GeoDataCoordinates::Vector::ConstIterator const & itStartPoint,
													GeoDataCoordinates::Vector::ConstIterator const & itEndPoint,
													const int detail, const ViewportParams *viewport, int offset )
{	
    ...
    if ( src.size() >= 2 ) {
		...
		for (int i=0; i<src.size(); i++){
			const QPointF v = src[i];
			qreal x0, y0;
			lambertProjection->screenCoordinates(v.x(), v.y(), viewport, proj_n, proj_F, proj_rho0, x0, y0);
			m << QPointF (x0, y0);
		}
        m_polygons.append(m);
    }
                                           ...
                                           const ViewportParams* viewport )
{
	...
	if (m_polygon.capacity()<256) m_polygon.reserve( 256 );
									   ...
									   int offset )
{
    ...
    const qreal rad2Pixel =  viewport->radius() / M_PI_2;
	const qreal vpwidth = viewport->width();
	const qreal vpheight = viewport->height();
	const qreal vpwidth05 = vpwidth * 0.5;
	const qreal vpheight05 = vpheight * 0.5;
    ...
    for (; itPoint != itEndPoint; ++itPoint ) {
        ...
        const qreal x = vpwidth05 + rad2Pixel * (lon - centerLon) + offset;
        const qreal y = vpheight05 - rad2Pixel * ( lat - centerLat );
        int currentSign = ( lon > 0.0 ) ? 1 : -1 ;
		if ( firstPoint ) {
			firstPoint = false;
            lastSign = currentSign;
        ...
        if ( lastSign != currentSign && fabs(lastLon) + fabs(lon) > M_PI ) {
			...
			qreal lastXAtDateLine = vpwidth05 + rad2Pixel * ( lastSign * M_PI - centerLon ) + offset;
			qreal xAtDateLine = vpwidth05 + rad2Pixel * ( -lastSign * M_PI - centerLon ) + offset;
			qreal lastYAtDateLine = vpheight05 - rad2Pixel* ( lastLat - centerLat ) ;
			qreal yAtSouthPole = vpheight05 - rad2Pixel * ( -viewport->currentProjection()->maxLat() - centerLat  );
			
			//If the "jump" occurs in the Anctartica's latitudes
			
			if ( lat < - M_PI / 3 ) {
				const qreal vpwidth1 = vpwidth - 1.0;
				// FIXME: This should actually need to get investigated
				//        in GeoPainter.  For now though we just help
				//        GeoPainter to get the clipping right.
				if ( lastXAtDateLine > vpwidth1)
					lastXAtDateLine = vpwidth1;
				if ( lastXAtDateLine < 0.0 )
					lastXAtDateLine = 0.0; 
				if ( xAtDateLine > vpwidth1 )
					xAtDateLine = vpwidth1;
				if ( xAtDateLine < 0.0 )
					xAtDateLine = 0.0; 
									   ...
									   int offset )
{
    ...
    for (; itPoint != itEndPoint; ++itPoint ) {
        ...
        const qreal x = vpwidth05 + rad2Pixel * (lon - centerLon) + offset;
        const qreal y = vpheight05 - rad2Pixel * ( atanh( sin( lat ) ) - atanh( sin( centerLat ) ) );
        int currentSign = ( lon > 0.0 ) ? 1 : -1 ;
		if ( firstPoint ) {
			firstPoint = false;
            lastSign = currentSign;
        ...
        if ( lastSign != currentSign && fabs(lastLon) + fabs(lon) > M_PI ) {
			...
			qreal lastXAtDateLine = vpwidth05 + rad2Pixel * ( lastSign * M_PI - centerLon ) + offset;
			qreal xAtDateLine = vpwidth05 + rad2Pixel * ( -lastSign * M_PI - centerLon ) + offset;
			qreal lastYAtDateLine = vpheight05 - rad2Pixel* ( atanh( sin( lastLat ) ) - atanh( sin( centerLat ) ) ) ;
			qreal yAtSouthPole = vpheight05 - rad2Pixel * ( atanh( sin( -viewport->currentProjection()->maxLat() ) ) - atanh( sin( centerLat ) ) );
