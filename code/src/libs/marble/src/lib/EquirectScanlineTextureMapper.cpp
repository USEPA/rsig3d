...
void EquirectScanlineTextureMapper::mapTexture( const ViewportParams *viewport, MapQuality mapQuality )
{
	...
	int yStep = ( yPaintedBottom - yPaintedTop ) / numThreads;
	if ( (yPaintedBottom - yPaintedTop)%numThreads != 0 ) yStep++;	
	
    for ( int i = 0; i < numThreads; ++i ) {
        const int yStart = yPaintedTop +  i * yStep;
        int yEnd = yPaintedTop + (i + 1) * yStep;
		if (yEnd>yPaintedBottom) yEnd = yPaintedBottom;
...
/*
 //this function is correct, but less general, so not used 
void EquirectScanlineTextureMapper::RenderJob::run()
{
    ...
    // Calculate how many degrees are being represented per pixel.
    const qreal MATH_PI_2 = M_PI * 0.5;
    const qreal MATH_2_PI = M_PI * 2.0;
    const qreal rad2Pixel = (qreal)(radius ) / MATH_PI_2;
    ...
    const int yTop = imageHeight / 2 - radius + yCenterOffset;
    const qreal leftLon = +centerLon - ( imageWidth / 2 * pixel2Rad );
    ...
    // Scanline based algorithm to do texture mapping
    const qreal npi = 129.0 * M_PI;
    for ( int y = m_yPaintedTop; y < m_yPaintedBottom; ++y ) {
		
        QRgb * scanLine = (QRgb*)( m_canvasImage->scanLine( y ) );
        const qreal lat =  ( imageHeight / 2  - y ) * pixel2Rad  + centerLat ;
        qreal lon = leftLon;
		
        for ( int x = 0; x < imageWidth; ++x ) {
            const qreal lon2 = fmod(lon + npi, MATH_2_PI) - M_PI;
            context.pixelValue( lon2, lat, scanLine );
            ++scanLine;
            lon += pixel2Rad;
        }
...
void EquirectScanlineTextureMapper::RenderJob::run()
{
    ...
    // Calculate how many degrees are being represented per pixel.
    const qreal rad2Pixel = 2.0 * radius / M_PI;
    ...
    while ( leftLon < -M_PI ) leftLon += MATH_2_PI;
    while ( leftLon >  M_PI ) leftLon -= MATH_2_PI;
    ...
    for ( int y = m_yPaintedTop; y < m_yPaintedBottom; ++y ) {
        ...
        qreal lon = leftLon;
        const qreal lat = ( ( ( (imageHeight / 2 + yCenterOffset) - y ) * pixel2Rad ) );
