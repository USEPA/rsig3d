...
void SphericalScanlineTextureMapper::mapTexture( const ViewportParams *viewport, MapQuality mapQuality )
{
    ...
    const int yStep = ( yBottom - yTop ) / numThreads + ((yBottom - yTop)%numThreads!=0);
...
void SphericalScanlineTextureMapper::RenderJob::run()
{
    ...
    qreal  lon = 0.0;
    qreal  lat = 0.0;
	const int imageWidth02 = imageWidth / 2;
    ...
    for ( int y = m_yTop; y < m_yBottom ; ++y ) {
        ...
        const int xLeft  = ( ( imageWidth02 - rx > 0 )
                             ? imageWidth02 - rx : 0 ); 
        const int xRight = ( ( imageWidth02 - rx > 0 )
                             ? xLeft + rx + rx : imageWidth );

        QRgb * scanLine = (QRgb*)( m_canvasImage->scanLine( y ) ) + xLeft;

        const int xIpLeft  = ( imageWidth02 - rx > 0 ) ? n * (int)( xLeft / n + 1 )
                                                         : 1;
        const int xIpRight = ( imageWidth02 - rx > 0 ) ? n * (int)( xRight / n - 1 )
                                                         : n * (int)( xRight / n - 1 ) + 1; 
...
//                mDebug() << QString("NorthPole X: %1, LeftInterval: %2").arg( northPoleX ).arg( leftInterval );
            ...
            // the current pixel.
            const qreal qx = (qreal)( x - imageWidth02 ) * inverseRadius;
