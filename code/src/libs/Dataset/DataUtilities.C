...
double toReal( const char* string, double lower, double upper, bool& ok ) {
  ...
  if ( *string ) {
    char* terminator = 0;
    const double conversionResult = strtod( string, &terminator );
    ...
    if ( AND2( terminator, terminator != string ) ) {
      ...
      ok = AND2( *terminator == '\0', // No remaining characters.
                 IN_RANGE( conversionResult, lower, upper ) );
...
INPUTS:  const double array[]  The array of values to check.
         const size_t count    The number of values in the array.
...
/******************************************************************************
PURPOSE: updateMissingValues - Replace any old missing values (-9999.0) with
         new MISSING value.
INPUTS:  double array[]      The array of values to check/replace.
         const size_t count  The number of values in the array.
******************************************************************************/
            ...
            if ( IMPLIES( extensions, strstr( extensions, extension + 1 ) ) ) {
              ...
              if ( ! stat( pathedFileName, &info ) ) {
                ...
                if ( AND3( year == todayYear, month == todayMonth,
                  ...
                  snprintf( line, LINE_LENGTH, "%9" LU " %02d:%02d %s\n",
