/******************************************************************************
PURPOSE: RegriddedSwathDataset.C - Dataset for regridded Swath satellite data.
...
RegriddedSwathDataset::RegriddedSwathDataset( FILE* stream ) {
  ...
  if ( ! invariant() ) {
    throw std::runtime_error( "Failed to read valid regridded Swath data."); //x
  }
...
RegriddedSwathDataset::RegriddedSwathDataset( const std::string& fileName ) {
  ...
  if ( ! invariant() ) {
    throw std::runtime_error( "Failed to read valid regridded Swath data "
...
Dataset* RegriddedSwathDataset::sample( const Dataset& other ) const {
  PRE( other.invariant() );
  Dataset* result = 0;
  throw std::runtime_error("UNIMPLEMENTED: RegriddedSwathDataset::sample()");//x
...
Dataset* RegriddedSwathDataset::regrid( const int regridMethod,
  ...
  Dataset* result = 0;
  throw std::runtime_error("UNIMPLEMENTED: RegriddedSwathDataset::regrid()");//x
  POST( IMPLIES( result, result->invariant() ) );
  return result;
}
...
void RegriddedSwathDataset::readXDR( FILE* stream ) {
  PRE0( stream );

  /* Read ASCII header that looks like this:

  REGRIDDED-Swath 2.0
  http://www.ssd.noaa.gov/PS/FIRE/GASP/gasp.html,GASPSubset,XDRConvert
  ...
  */
  ...
  readAndMatchLine( stream, "REGRIDDED-Swath 2.0\n" ); //x
  ...
  CHECK2( variables_ == 3, dataVariable_ == 2 );

  // HACK for satellite AOD to PM25:

  if ( AND2( timestepSize_ != DAYS,
             ! strcmp( variableNames_[ dataVariable_ ].c_str(), "PM25" ) ) ) {
    timestepSize_ = DAYS;
  }
...
void RegriddedSwathDataset::writeXDR( FILE* stream ) const {
  PRE( stream );
  ...
  */
  ...
  if ( ! ok ) {
    throw std::runtime_error( "Failed to write XDR regridded Swath data."); //x
  }
