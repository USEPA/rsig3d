...
namespace qmapcontrol
{
    ...
    public:
        ...
        QPointF screenToWorldCoordinate (const QPoint& click ) const
		{
			return clickToWorldCoordinate(click);
		}

		//World long-lat coordinate (in degree) to screen coordinate		
		QPoint worldCoordinateToScreen(const QPointF& uv) const;
		
		void getWorldCoordinateToScreenParameters(float screen_middle[2], float midp[2], float& numberOfTiles, float& mytilesize) const;
		
    private:		
        QPointF clickToWorldCoordinate (const QPoint& click ) const;
